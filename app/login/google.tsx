...
export default function Google() {
  ...
  const mutation = useMutation(
    async () => {
      ...
      // @ts-ignore
      const crsfNode = query.data.ui.nodes.find((node) => {
      ...
      await kratos.updateLoginFlow({
        ...
        updateLoginFlowBody: {
          method: 'oidc',
          csrf_token:
            'value' in crsfNode?.attributes ? crsfNode?.attributes.value : '',
          provider: 'google'
        }
      });
    },
  ...
  return (
    ...
    >
      <div className={'flex justify-start items-center w-64 gap-4'}>
        ...
        />
        <span className={'mr-auto ml-auto'}>Continue with Google</span>
      </div>
    </Button>
  );
}
